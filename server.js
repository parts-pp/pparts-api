...
async function getOrCreateClientUser({
    ...
    const { data: created, error: createError } = await supabase
        ...
        .insert({
            ...
            last_name: full_name
                ? full_name.split(" ").slice(1).join(" ") || null
                : null,
            role: "client",
...
app.post("/auth/register-client", async (req, res) => {
    try {
        ...
        const phoneOtp = await createOtpRecord({
            user_id: user.id,
            channel: "phone",
            target_value: phone,
            purpose: "register",
        ...
        return res.status(201).json({
            success: true,
            message: "تم إنشاء الحساب وإرسال أكواد التحقق",
            user: {
                id: user.id,
...
app.post("/auth/register-trader", async (req, res) => {
    try {
        ...
        return res.status(201).json({
            success: true,
            message: "تم تسجيل التاجر وإرسال الطلب للمراجعة وأكواد التحقق",
            user: {
                id: user.id,
                public_id: user.public_id,
                full_name: user.full_name,
...
app.post("/auth/login", async (req, res) => {
    try {
        ...
        const requiresVerification = !user.is_phone_verified || !user.is_email_verified;
        ...
        if (requiresVerification) {
            ...
            return res.status(403).json({
                ...
                message: "يجب التحقق من الحساب أولًا",
                user: {
                    id: user.id,
                    full_name: user.full_name,
                    phone: user.phone,
                ...
                verification: {
                    phone_required: !user.is_phone_verified,
                    email_required: !user.is_email_verified,
                    phone_demo_code:
                        otpRecords.find((x) => x.channel === "phone")?.code || null,
                    email_demo_code:
                        otpRecords.find((x) => x.channel === "email")?.code || null,
                },
            });
        }
...
app.post("/auth/verify", async (req, res) => {
    try {
        ...
        const userUpdate = {};
        if (channel === "phone") userUpdate.is_phone_verified = true;
        if (channel === "email") userUpdate.is_email_verified = true;
        ...
        return res.json({
            ...
            verification: {
                is_phone_verified: channel === "phone" ? true : user.is_phone_verified,
                is_email_verified: channel === "email" ? true : user.is_email_verified,
            },
        });
    } catch (err) {
