...
const OTP_ALLOWED_PURPOSES = ["register", "login", "resend"];
const OTP_EXPIRY_MINUTES = 10;
const OTP_RESEND_COOLDOWN_SECONDS = 60;
const OTP_MAX_VERIFY_ATTEMPTS = 3;
const OTP_MAX_REQUESTS_PER_USER_PER_HOUR = 5;
const OTP_MAX_REQUESTS_PER_IP_PER_HOUR = 10;
...
function sanitizeOtpPurpose(purpose) {
    const safePurpose = String(purpose || "").trim().toLowerCase();
    return OTP_ALLOWED_PURPOSES.includes(safePurpose) ? safePurpose : "register";
}
...
async function sendOtpEmail({ to, code }) {
    ...
    const { data, error } = await resend.emails.send({
      ...
      <div style="font-family: Arial, sans-serif; direction: rtl; text-align: right; line-height:1.8;">
        ...
        <div style="font-size:32px; font-weight:bold; margin:16px 0; letter-spacing:4px;">${code}</div>
        <p style="margin:0 0 8px;">صلاحية الرمز ${OTP_EXPIRY_MINUTES} دقائق.</p>
        <p style="margin:0; color:#666;">إذا لم تطلب هذا الرمز، يمكنك تجاهل الرسالة.</p>
      </div>
    `,
    });
...
async function createOtpRecord({
    ...
    const safePurpose = sanitizeOtpPurpose(purpose);

    if (channel !== "email") {
        throw new Error("التحقق المتاح حاليًا عبر البريد الإلكتروني فقط");
    }

    const requestIp = req ? getRequestIp(req) : null;
    const now = Date.now();
    const oneHourAgoIso = new Date(now - 1000 * 60 * 60).toISOString();
    ...
    if ((recentRows || []).length >= OTP_MAX_REQUESTS_PER_USER_PER_HOUR) {
        throw new Error("تم تجاوز الحد المسموح لطلبات الرمز، حاول لاحقًا");
    }

    const latest = recentRows?.[0];
    if (latest?.last_sent_at) {
        const diffMs = now - new Date(latest.last_sent_at).getTime();
        if (diffMs < OTP_RESEND_COOLDOWN_SECONDS * 1000) {
            const waitSec = Math.ceil(
                (OTP_RESEND_COOLDOWN_SECONDS * 1000 - diffMs) / 1000
            );
            throw new Error(`انتظر ${waitSec} ثانية قبل إعادة إرسال الرمز`);
        }
    }
    ...
    if (requestIp) {
        ...
        if ((ipRows || []).length >= OTP_MAX_REQUESTS_PER_IP_PER_HOUR) {
            throw new Error("تم تجاوز الحد المسموح من هذا الاتصال، حاول لاحقًا");
        }
    }
    ...
    const expiresAt = new Date(
        now + 1000 * 60 * OTP_EXPIRY_MINUTES
    ).toISOString();
    ...
    const { data, error } = await supabase
        ...
        .insert({
            ...
            attempts_count: 0,
            resend_count: safePurpose === "resend" ? 1 : 0,
            last_sent_at: nowIso(),
    ...
    await writeAuditLog({
        ...
        actor_role: null,
        action_type: safePurpose === "resend" ? "resend_otp" : "create_otp",
...
async function getSessionWithUser(sessionToken) {
    ...
    if (new Date(data.expires_at).getTime() < Date.now()) {
        await supabase.from("user_sessions").update({ is_revoked: true }).eq("id", data.id);
        return null;
    }

    await supabase.from("user_sessions").update({ last_seen_at: nowIso() }).eq("id", data.id);
...
async function getOrCreateClientUser({
    ...
    if (telegram_id) {
        ...
        query = supabase.from("users").select("*").eq("phone", phone).limit(1);
    }
...
app.post("/auth/register-trader", async (req, res) => {
    try {
        ...
        return res.status(201).json({
            success: true,
            message: "تم تسجيل التاجر وإرسال الطلب للمراجعة ورمز التحقق إلى البريد",
            user: {
                id: user.id,
                public_id: user.public_id,
                full_name: user.full_name,
...
app.post("/auth/verify", async (req, res) => {
    try {
        ...
        if ((otp.attempts_count || 0) >= OTP_MAX_VERIFY_ATTEMPTS) {
            await supabase
                .from("otp_codes")
                .update({ invalidated_at: nowIso() })
                .eq("id", otp.id);
        ...
        if (incomingHash !== otp.code_hash) {
            const nextAttempts = (otp.attempts_count || 0) + 1;

            await supabase
                .from("otp_codes")
                .update({
                    attempts_count: nextAttempts,
                    invalidated_at: nextAttempts >= OTP_MAX_VERIFY_ATTEMPTS ? nowIso() : null,
                })
                .eq("id", otp.id);
        ...
        await supabase.from("otp_codes").update({ verified_at: nowIso() }).eq("id", otp.id);
...
app.get("/auth/me", async (req, res) => {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: err.message || "تعذر جلب بيانات الجلسة",
        });
    }
});
...
app.post("/auth/logout", async (req, res) => {
    try {
        ...
        if (!token) {
            return res.status(400).json({
                success: false,
                message: "لا توجد جلسة لتسجيل الخروج",
            });
        }
        ...
        return res.status(500).json({
            success: false,
            message: err.message || "تعذر تسجيل الخروج",
        });
    }
});
